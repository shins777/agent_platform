Please build a Google ADK (Agent Development Kit) Search Agent platform on Google Cloud Platform with Vertex AI Agent Engine and GCP Agent Registry A2A protocol support. Implement all the following files step by step under `agent_gateway/build_agent/`:

1. `agent_gateway/build_agent/agent.py`:
...
2. `agent_gateway/build_agent/deploy_agent.py`:
- Implement a 3-step deployment pipeline:
  - Step 1 (`step1_get_adk_agent`): Load `root_agent` and `a2a_app` from `agent_gateway.build_agent.agent`.
...
3. `agent_gateway/build_agent/deploy.sh`:
...
4. `agent_gateway/build_agent/call_agent_directly.py`:
...
5. `agent_gateway/build_agent/call_agent_registry.py`:
...
6. `agent_gateway/build_agent/README.md`:
